 ...
 *
 * issue-intake runs on `issues: opened` and assigns every new issue an
 * outcome (label `ready-for-pilot` / `needs-info` / `needs-decomposition`,
 * or close). But if that run *fails* — e.g. a transient claude-code-action
 * internal error — the issue is left with NO intake label, no comment, and
 * silent. Pilot agents only pick labelled issues, so an un-triaged issue is
 * invisible and the reporter never hears back.
 *
